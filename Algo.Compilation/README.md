...
**Algo.Compilation** provides runtime code compilation services for the StockSharp trading platform.
It enables you to compile trading algorithms and analytics scripts written in different languages, including C#, Visual Basic, F#, and Python, directly from your application.
...
- **Dynamic script execution** – Compile and execute code snippets or whole modules at runtime.
  This allows building strategies and analytics that can be modified without recompiling the entire application.
...
you must call `CompilationExtensions.Init` once during application startup:
...
The `CodeInfo` class represents a source file and contains helper methods to compile it.
...
`CodeInfo` manages assembly, project, and NuGet references.
It also caches compiled assemblies when the compiler supports it.
